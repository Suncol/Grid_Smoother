...
def plotBarQuality(skewnessData, title):
    ...
    # # Save figure ? Just show the figure now 
    # fName = "results/" + title + ".png"
    # fig.savefig(fName, dpi=150)


def plotGrid(X,Y,charTitle = '',lineColor='b',lineWidth=1,
    ...
    plot: To plot structured grid
        ...
        INPUT:
            X (matrix)      - matrix with x-coordinates of gridpoints / mesh grids
            Y (matrix)      - matrix with y-coordinates of gridpoints / mesh grids
    ...
    xdiv, ydiv = X.shape # get the mesh grid, that of X and Y should be the same

    # Generating plot
    fig = plt.figure(figsize=(12,9))
    ax = plt.subplot(111)

    # plot scatter for the mesh points
    ...
    # plot lines
    ## link all point same x diff y
    for i in range(xdiv):
        ax.plot(X[i,:], Y[i,:], lineColor, lineWidth)
    
    ## link all point same y diff x
    for j in range(ydiv):
